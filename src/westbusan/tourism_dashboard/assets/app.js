    ...
    [["west", -barWidth - barGap / 2], ["east", barGap / 2]].forEach(([region, offset]) => {
      ...
      if (amount > 0) {
        const valueLabel = svgNode("text", {
          ...
          class: region === "west" ? "trend-west-entry-label" : "trend-east-entry-label",
          "text-anchor": "middle",
        });
...
function renderDashboard(data) {
  ...
  overview.append(
    ...
    kpi("객실 100실당 방문수요", value(west.demandPer100Rooms), "일평균 방문수요 ÷ 확인 객실 × 100", relativeToEast(west.demandPer100Rooms, east.demandPer100Rooms), "서부산 일평균 외지인·외국인 방문수요를 객실 수가 확인된 객실 합계로 나눈 뒤 100을 곱한 공급압력 지표입니다. 실제 숙박객 수·객실 가동률이 아닙니다."),
