...
3. MCP는 로컬에서 `v4.12.0` 소스 빌드, `/health`, `tools/list`, `legal_research` 노출까지 확인했다. 운영 `LAW_OC` 자격증명이 현재 작업 셸에 없으므로 실제 법령조회 성공은 확인하지 않았다.
...
2. 현재 운영 DB를 백업하고 migration 048을 적용한다.
3. 승인된 낙동강 필지규제 발행본의 PNU 집합을 UTF-8 파일로 추출하고 목표 건수 및 집합 해시를 기록한다.
4. 서버 전용 VWorld 자격증명으로 다음 명령을 실행한다.
...
5. `target_count`, `matched_count`, `coverage`와 실패상태별 건수를 확인한다. 필지규제 현재 발행본과 필지도형 현재 발행본의 PNU 집합 차이가 0인지 별도 검사한다.
6. Korean Law MCP `v4.12.0`, 커밋 `7839d70a2b9e336ac47c70eeef64fc4714970224`를 전용 release에 설치하고 Node.js 20.19.0 이상인지 확인한다.
7. MCP·AI용 환경파일은 서버 내부 `0640` 파일로 만들고 systemd 전용 계정만 읽게 한다. 환경값을 터미널·로그·문서에 출력하지 않는다.
8. MCP `/health`와 허용 도구를 loopback에서 확인한 뒤 관광 AI 서비스를 재시작한다.
9. Nginx 설정검사 후 공개 정책해설 경로만 반영한다. MCP 18082 포트가 외부에서 접근되지 않는지 확인한다.
10. 실제 필지 내부·필지 경계·발행범위 밖 3개 좌표를 시험하고, 결정등급이 AI 응답 전후 동일한지 확인한다.
11. 기존 관광 대시보드·투자지도·빈집지도·낙동강 지도와 `/tourism/api/healthz`가 모두 HTTP 200인지 확인한 뒤 current release를 전진한다.
